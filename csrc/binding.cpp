...
torch::Tensor ctct_loss_fwd(const torch::Tensor &log_probs,
    ...
    auto alphas = torch::empty({N, T - U + 1, 2 * U + 1}, log_probs.options());
    auto betas = torch::empty_like(alphas);
    ...
    run_warp_ctct(
        ...
        (const int *)ly.data_ptr<int>(), (int *)counts.data_ptr<int>(), N, T,
        U);
